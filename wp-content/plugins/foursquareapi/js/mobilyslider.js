...
(function($){
	$.fn.mobilyslider=function(options){
		...
		return this.each(function(){
			var $t=$(this),item=$t.children(sets.content).children(sets.items),l=item.length-1,w=item.width(),h=item.height(),step=0,play,bullets,arrows,z,active,bullet;
			var slider={
				init:function(){
					slider.data();
					if(sets.bullets){
					...
					item.eq(step).css("z-index",l+4).next(item).css("z-index",l+2)},next:function(){
					...
					create:function(){
						$t.append($("<div />").attr('id', sets.arrowsClass+sets.id).addClass(sets.arrowsClass));
						arrows=$t.find("#"+sets.arrowsClass+sets.id);
						arrows.append($("<a />").attr({ href: "#", id: sets.prev+sets.id }).addClass(sets.prev).text("Previous"));
						arrows.append($("<a />").attr({ href: "#", id: sets.prev+sets.id }).addClass(sets.next).text("Next"))
					},trigger:function(){
						arrows.find("#"+sets.prev+sets.id).click(function(){
							slider.animation.previous();
							if(sets.bullets){
								slider.bullets.update()
							}return false
						});
						arrows.find("#"+sets.next+sets.id).click(function(){
							slider.animation.next();
							if(sets.bullets){
								slider.bullets.update()
							}return false
